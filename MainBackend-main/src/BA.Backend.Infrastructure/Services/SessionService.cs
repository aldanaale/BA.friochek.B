...
public class SessionService(DatabaseSettings settings, ILogger<SessionService> logger) : ISessionService
{
    public async Task RegisterSessionAsync(string sessionId, Guid userId, DateTime expiresAt, CancellationToken ct)
    {
        logger.LogInformation("Registrando sesión activa en Dapper para el usuario: {UserId}", userId);
    ...
    {
        logger.LogInformation("Revocando (cerrando) la sesión: {SessionId}", sessionId);
