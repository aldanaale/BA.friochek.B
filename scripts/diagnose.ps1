...
function Get-Token($cred) {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/login" -Body @{
...
Test-Case "C01" "GET /api/v1/ping retorna 200 Online" {
    $r = Invoke-Api -Url "$BASE/api/v1/ping"
...
Test-Case "C07" "Header CORS presente con Origin cross-origin" {
    try {
        ...
        $resp = Invoke-WebRequest -Method GET -Uri "$BASE/api/v1/ping" -Headers $headers -UseBasicParsing -ErrorAction Stop
...
Test-Case "A18" "Token expirado/invalido en endpoint protegido retorna 401" {
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.fake.fake"
    @{ ok=($r.StatusCode -eq 401); detail="Status:$($r.StatusCode)" }
}

Test-Case "A19" "Sin token en endpoint protegido retorna 401" {
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard"
    @{ ok=($r.StatusCode -eq 401); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "A20" "POST /auth/logout con token valido retorna 200" {
    ...
    $loginR = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/login" -Body @{
        email=$CREDS["Tec1Savory"].email; password=$SQL_PASS; tenantSlug=$DB.T1.Slug
    }
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/logout" -Token $tmpToken
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "A21" "POST /auth/forgot-password con email+slug validos retorna 200" {
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/forgot-password" -Body @{
        email=$CREDS["AdminSavory"].email; tenantSlug=$DB.T1.Slug
    }
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "S05" "GET /users retorna paginacion con totalCount, pageNumber, pageSize, items" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=1`&pageSize=5" -Token $TOKENS["AdminSavory"]
    $d = $r.Body.data
...
Test-Case "S06" "Pagina inexistente retorna 200 con totalCount valido" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=999`&pageSize=10" -Token $TOKENS["AdminSavory"]
...
Test-Case "S07" "POST /stores retorna 201 con Location header" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/stores" -Token $TOKENS["AdminSavory"] -Body @{
...
Test-Case "S08" "POST /users retorna 201 con Location header" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/users" -Token $TOKENS["AdminSavory"] -Body @{
...
Test-Case "S09" "Respuestas de error DomainException retornan 400 (no 500)" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/cliente/orders/$fakeId/confirm" -Token $TOKENS["Cliente1Savory"]
...
Test-Case "S10" "Content-Type application/json en error 401" {
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token "fake"
...
Test-Case "R01" "Admin accede a GET /admin/dashboard 200" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "R02" "Admin accede a GET /stores 200" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/stores" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "R03" "Admin accede a GET /users 200" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "R04" "Cliente NO accede a GET /admin/dashboard (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token $TOKENS["Cliente1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}

Test-Case "R05" "Cliente NO accede a GET /stores (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/stores" -Token $TOKENS["Cliente1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}

Test-Case "R06" "Cliente NO accede a GET /users (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users" -Token $TOKENS["Cliente1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}

Test-Case "R07" "Cliente NO puede POST /users (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/users" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "R08" "Cliente NO puede POST /stores (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/stores" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "R09" "Tecnico NO accede a GET /admin/dashboard (403)" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token $TOKENS["Tec1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}

Test-Case "R10" "Tecnico NO accede a GET /users (403)" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users" -Token $TOKENS["Tec1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "R11" "Tecnico accede a GET /tecnico/tickets 200 o lista vacia" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $fakeId = [Guid]::NewGuid()
    $r = Invoke-Api -Url "$BASE/api/v1/tecnico/tickets?tecnicoId=$fakeId" -Token $TOKENS["Tec1Savory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "R12" "Cliente NO accede a GET /tecnico/tickets (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/tecnico/tickets?tecnicoId=$([Guid]::NewGuid())" -Token $TOKENS["Cliente1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "R13" "Transportista accede a GET /transportista/route (requiere fix SQL o.Total)" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/transportista/route" -Token $TOKENS["Trans1Savory"]
...
Test-Case "MT04" "GET /cliente/products filtra por tenant correcto" {
    if (-not $TOKENS["Cliente1Coppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/products" -Token $TOKENS["Cliente1Coppelia"]
...
Test-Case "MT05" "Admin tenant A no puede ver usuario de tenant B por ID" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/users/$pepsiUserId" -Token $TOKENS["AdminSavory"]
...
Test-Case "U07" "Tecnico NO puede crear usuario (403)" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/users" -Token $TOKENS["Tec1Savory"] -Body @{
        email="hack@test.com"; fullName="Hacker"; password=$SQL_PASS; role=1
    }
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "U08" "Admin GET /users/{id} retorna usuario existente 200" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/users/$CREATED_USER_ID" -Token $TOKENS["AdminSavory"]
...
Test-Case "U09" "Admin GET /users/{id} inexistente retorna 404" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users/99999999-9999-9999-9999-999999999999" -Token $TOKENS["AdminSavory"]
...
Test-Case "U10" "Admin PUT /users/{id} actualiza fullName 200" {
    ...
    $r = Invoke-Api -Method PUT -Url "$BASE/api/v1/users/$CREATED_USER_ID" -Token $TOKENS["AdminSavory"] -Body @{
...
Test-Case "U11" "Admin PUT /users/{id} usuario de otro tenant retorna 400 o 404" {
    ...
    $r = Invoke-Api -Method PUT -Url "$BASE/api/v1/users/$CREATED_USER_ID" -Token $TOKENS["AdminCoppelia"] -Body @{
...
Test-Case "U12" "Cliente NO puede actualizar usuario (403)" {
    ...
    $r = Invoke-Api -Method PUT -Url "$BASE/api/v1/users/$CREATED_USER_ID" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "U13" "Admin POST /users/{id}/lock bloquea usuario 200" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/users/$CREATED_USER_ID/lock" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "U14" "Admin POST /users/{id}/unlock desbloquea usuario 200" {
    if (-not $TOKENS["AdminSavory"] -or -not $CREATED_USER_ID) { return @{ ok=$false; detail="Sin ID" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/users/$CREATED_USER_ID/unlock" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "U15" "Cliente NO puede bloquear usuario (403)" {
    if (-not $TOKENS["Cliente1Savory"] -or -not $CREATED_USER_ID) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/users/$CREATED_USER_ID/lock" -Token $TOKENS["Cliente1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}

Test-Case "U16" "Admin DELETE /users/{id} soft-delete 200" {
    if (-not $TOKENS["AdminSavory"] -or -not $CREATED_USER_ID) { return @{ ok=$false; detail="Sin ID" } }
    $r = Invoke-Api -Method DELETE -Url "$BASE/api/v1/users/$CREATED_USER_ID" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "U17" "Admin GET /users paginado retorna pageNumber y pageSize correctos" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=2`&pageSize=3" -Token $TOKENS["AdminSavory"]
...
Test-Case "ST04" "Admin GET /stores/{id} tienda existente 200" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/stores/$CREATED_STORE_ID" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "ST05" "Admin GET /stores/{id} inexistente retorna 404" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/stores/99999999-9999-9999-9999-999999999999" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 404); detail="Status:$($r.StatusCode)" }
}

Test-Case "ST06" "Admin PUT /stores/{id} actualiza datos 200" {
    if (-not $TOKENS["AdminSavory"] -or -not $CREATED_STORE_ID) { return @{ ok=$false; detail="Sin ID" } }
    $r = Invoke-Api -Method PUT -Url "$BASE/api/v1/stores/$CREATED_STORE_ID" -Token $TOKENS["AdminSavory"] -Body @{
...
Test-Case "ST07" "Admin PUT /stores/{id} de otro tenant retorna 404" {
    ...
    $r = Invoke-Api -Method PUT -Url "$BASE/api/v1/stores/$CREATED_STORE_ID" -Token $TOKENS["AdminCoppelia"] -Body @{
...
Test-Case "ST09" "Tecnico NO puede actualizar tienda (403)" {
    ...
    $r = Invoke-Api -Method PUT -Url "$BASE/api/v1/stores/$CREATED_STORE_ID" -Token $TOKENS["Tec1Savory"] -Body @{
...
Test-Case "ST10" "Admin DELETE /stores/{id} elimina tienda 200" {
    if (-not $TOKENS["AdminSavory"] -or -not $CREATED_STORE_ID) { return @{ ok=$false; detail="Sin ID" } }
    $r = Invoke-Api -Method DELETE -Url "$BASE/api/v1/stores/$CREATED_STORE_ID" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "ST11" "Admin DELETE /stores/{id} inexistente retorna 404" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method DELETE -Url "$BASE/api/v1/stores/99999999-9999-9999-9999-999999999988" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 404); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "N01" "POST /nfc/validate con UID inexistente retorna 404" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/nfc/validate" -Token $TOKENS["AdminSavory"] -Body @{
...
Test-Case "N02" "POST /nfc/validate con UID real de BD retorna 200 + accessToken" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/nfc/validate" -Token $TOKENS["Tec1Savory"] -Body @{
        nfcUid = $uid
    }
...
Test-Case "N03" "POST /nfc/enroll con cooler inexistente retorna 404 o 400" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/nfc/enroll" -Token $TOKENS["AdminSavory"] -Body @{
...
Test-Case "N04" "Cliente NO puede hacer POST /nfc/enroll (403 o 400 por tenant)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/nfc/enroll" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "N05" "Sin token POST /nfc/validate retorna 401" {
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/nfc/validate" -Body @{
...
Test-Case "CL04" "GET /cliente/orders paginado retorna 200 con estructura" {
    if (-not $TOKENS["Cliente1Coppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/orders?pageNumber=1`&pageSize=10" -Token $TOKENS["Cliente1Coppelia"]
...
Test-Case "CL05" "GET /cliente/orders/{id} inexistente retorna 404" {
    if (-not $TOKENS["Cliente1Coppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/orders/99999999-9999-9999-9999-999999999999" -Token $TOKENS["Cliente1Coppelia"]
    @{ ok=($r.StatusCode -eq 404); detail="Status:$($r.StatusCode)" }
}

Test-Case "CL06" "GET /cliente/tech-support retorna 200 paginado" {
    if (-not $TOKENS["Cliente1Coppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/tech-support?pageNumber=1`&pageSize=10" -Token $TOKENS["Cliente1Coppelia"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "CL07" "POST /cliente/orders con nfcToken invalido retorna 401 o 400" {
    if (-not $TOKENS["Cliente1Coppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/cliente/orders" -Token $TOKENS["Cliente1Coppelia"] -Body @{
...
Test-Case "CL08" "POST /cliente/orders/{id}/items en orden inexistente retorna 404 o 400" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/cliente/orders/$fakeId/items" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "CL09" "POST /cliente/nfc/report-damaged cooler inexistente retorna 404 o 400" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/cliente/nfc/report-damaged" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "T01" "GET /tecnico/tickets con tecnicoId valido retorna 200" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/tecnico/tickets?tecnicoId=$([Guid]::NewGuid())" -Token $TOKENS["Tec1Savory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "T02" "GET /tecnico/tickets sin tecnicoId retorna 200 o 400" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/tecnico/tickets?tecnicoId=$([Guid]::Empty)" -Token $TOKENS["Tec1Savory"]
    @{ ok=($r.StatusCode -in @(200,400)); detail="Status:$($r.StatusCode)" }
}

Test-Case "T03" "POST /tecnico/falla retorna 200 con RegistroActividad" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/tecnico/falla" -Token $TOKENS["Tec1Savory"] -Body @{
...
Test-Case "T04" "POST /tecnico/re-enroll - BUG: FK constraint con CoolerId inexistente" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/tecnico/re-enroll" -Token $TOKENS["Tec1Savory"] -Body @{
...
Test-Case "T05" "Cliente NO puede acceder a POST /tecnico/falla (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/tecnico/falla" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "TR03" "POST /transportista/delivery con nfcToken invalido retorna 401 o 400" {
    if (-not $TOKENS["Trans1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/transportista/delivery" -Token $TOKENS["Trans1Savory"] -Body @{
...
Test-Case "TR05" "Tecnico NO puede registrar delivery (403)" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/transportista/delivery" -Token $TOKENS["Tec1Savory"] -Body @{
...
Test-Case "AD02" "Dashboard Admin Pepsi NO muestra datos de Coca-Cola" {
    if (-not $TOKENS["AdminCoppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $rPepsi = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token $TOKENS["AdminCoppelia"]
    $rCoca  = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token $TOKENS["AdminSavory"]
...
Test-Case "J04" "Token expirado/alterado retorna 401 en endpoint protegido" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token $fakeToken
    @{ ok=($r.StatusCode -eq 401); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "J06" "Token de sesion invalida retorna 401 en endpoint protegido" {
    ...
    # Logout
    Invoke-Api -Method POST -Url "$BASE/api/v1/auth/logout" -Token $token | Out-Null
    # Usar token revocado
    $r = Invoke-Api -Url "$BASE/api/v1/tecnico/tickets?tecnicoId=$([Guid]::NewGuid())" -Token $token
...
Test-Case "EX03" "KeyNotFoundException -> 404 (NO 500) - Fix F6.3" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/orders/99999999-9999-9999-9999-111111111111" -Token $TOKENS["Cliente1Savory"]
...
Test-Case "EP01" "POST /api/v1/auth/login existe y responde" {
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/login" -Body @{}
    @{ ok=($r.StatusCode -ne 404); detail="Status:$($r.StatusCode)" }
}

Test-Case "EP02" "POST /api/v1/auth/logout existe y requiere auth" {
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/logout"
    @{ ok=($r.StatusCode -eq 401); detail="Status:$($r.StatusCode)" }
}

Test-Case "EP03" "POST /api/v1/auth/forgot-password existe (AllowAnonymous)" {
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/forgot-password" -Body @{
        email="noexiste_ps_test@test.cl"; tenantSlug=$DB.T1.Slug
    }
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
...
Test-Case "EP04" "GET /api/v1/ping existe (AllowAnonymous)" {
    $r = Invoke-Api -Url "$BASE/api/v1/ping"
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "EP07" "GET /api/v1/admin/users NO existe (es /api/v1/users)" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/users" -Token $TOKENS["AdminSavory"]
    # Segun docs la ruta es /api/v1/users no /admin/users
    @{ ok=($r.StatusCode -eq 404); detail="Status:$($r.StatusCode) - docs desactualizados lo llaman /admin/users" }
}

Test-Case "EP08" "GET /api/v1/cliente/support NO existe (es /cliente/tech-support)" {
    if (-not $TOKENS["Cliente1Coppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/support" -Token $TOKENS["Cliente1Coppelia"]
...
$cleanToken = $null
$reLogin = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/login" -Body @{
    email=$CREDS["AdminSavory"].email; password=$SQL_PASS; tenantSlug=$DB.T1.Slug
}
...
Test-Case "RV03" "[POST-CLEAN] Seed de usuarios intacto" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=1`&pageSize=5" -Token $cleanToken
...
Test-Case "RV04" "[POST-CLEAN] Seed de stores intacto" {
    if (-not $cleanToken) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/stores" -Token $cleanToken
...
Test-Case "RV08" "[POST-CLEAN] CRUD completo (create -> read -> update -> delete)" {
    ...
    $ts = Get-Date -Format "HHmmssfff"
    # Create
    $c = Invoke-Api -Method POST -Url "$BASE/api/v1/stores" -Token $cleanToken -Body @{
    ...
    $id = $c.Body.data.id
    # Read
    $r = Invoke-Api -Url "$BASE/api/v1/stores/$id" -Token $cleanToken
    # Update
    $u = Invoke-Api -Method PUT -Url "$BASE/api/v1/stores/$id" -Token $cleanToken -Body @{
        name="RV-$ts-upd"; address="Upd"; contactName="RV"; isActive=$true
    }
    # Delete
    $d = Invoke-Api -Method DELETE -Url "$BASE/api/v1/stores/$id" -Token $cleanToken
...
Test-Case "RV09" "[POST-CLEAN] JWT invalida correctamente" {
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token "fake.token.post.clean"
...
if ($cleanToken) {
    $page = 1; $cleaned = 0
    do {
        $allUsers = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=$page`&pageSize=100" -Token $cleanToken
        ...
        foreach ($u in $toDelete) {
            Invoke-Api -Method DELETE -Url "$BASE/api/v1/users/$($u.id)" -Token $cleanToken | Out-Null
            $cleaned++
        }
...
Test-Case "RV10" "[POST-CLEAN] Repositorio sin datos residuales de test" {
    if (-not $cleanToken) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=1`&pageSize=100" -Token $cleanToken
    if ($r.StatusCode -ne 200) { return @{ ok=$false; detail="Status:$($r.StatusCode)" } }
...
Test-Case "CO01" "Admin GET /coolers lista coolers del tenant 200" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/coolers" -Token $TOKENS["AdminSavory"]
...
Test-Case "CO02" "Admin POST /coolers crea cooler 201" {
    ...
    $storeId = if ($DB.Stores[$slug]) { $DB.Stores[$slug] } else {
        $storeR = Invoke-Api -Url "$BASE/api/v1/stores" -Token $TOKENS["AdminSavory"]
    ...
    $ts = Get-Date -Format "HHmmssfff"
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/coolers" -Token $TOKENS["AdminSavory"] -Body @{
...
Test-Case "CO03" "Admin GET /coolers/{id} retorna cooler existente 200" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/coolers/$CREATED_COOLER_ID" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "CO04" "Admin GET /coolers/{id} inexistente retorna 404" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/coolers/99999999-9999-9999-9999-999999999994" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 404); detail="Status:$($r.StatusCode)" }
}

Test-Case "CO05" "Admin PUT /coolers/{id} actualiza cooler 200" {
    if (-not $TOKENS["AdminSavory"] -or -not $CREATED_COOLER_ID) { return @{ ok=$false; detail="Sin ID" } }
    $r = Invoke-Api -Method PUT -Url "$BASE/api/v1/coolers/$CREATED_COOLER_ID" -Token $TOKENS["AdminSavory"] -Body @{
        name="Cooler Refactorizado"; serialNumber="SN-UPD-123"; model="PS-Cooler-UPDATED"; capacity=600; status="Activo"
    }
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "CO06" "Cliente NO puede crear cooler (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/coolers" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "CO07" "Tecnico NO puede crear cooler (403)" {
    if (-not $TOKENS["Tec1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/coolers" -Token $TOKENS["Tec1Savory"] -Body @{
...
Test-Case "CO08" "Admin Pepsi NO ve coolers de Coca (aislamiento tenant)" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/coolers/$CREATED_COOLER_ID" -Token $TOKENS["AdminCoppelia"]
...
Test-Case "CO09" "Admin DELETE /coolers/{id} elimina cooler 200" {
    if (-not $TOKENS["AdminSavory"] -or -not $CREATED_COOLER_ID) { return @{ ok=$false; detail="Sin ID" } }
    $r = Invoke-Api -Method DELETE -Url "$BASE/api/v1/coolers/$CREATED_COOLER_ID" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "CO10" "Admin GET /coolers sin token retorna 401" {
    $r = Invoke-Api -Url "$BASE/api/v1/coolers"
    @{ ok=($r.StatusCode -eq 401); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "MS01" "GET /admin/mermas retorna 200 con lista paginada" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/mermas?pageNumber=1`&pageSize=10" -Token $TOKENS["AdminSavory"]
...
Test-Case "MS02" "Cliente NO puede acceder a GET /admin/mermas (403)" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/mermas?pageNumber=1`&pageSize=10" -Token $TOKENS["Cliente1Savory"]
    @{ ok=($r.StatusCode -eq 403); detail="Status:$($r.StatusCode)" }
}
...
Test-Case "MS03" "POST /cliente/tech-support crear ticket retorna 200 o 201" {
    ...
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/cliente/tech-support-json" -Token $TOKENS["Cliente1Savory"] -Body @{
...
Test-Case "MS04" "Admin GET /admin/tech-support lista tickets del tenant 200" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/tech-support?pageNumber=1`&pageSize=10" -Token $TOKENS["AdminSavory"]
    @{ ok=($r.StatusCode -eq 200); detail="Status:$($r.StatusCode)" }
}

Test-Case "MS05" "Admin Pepsi NO ve tickets de Coca (aislamiento tenant)" {
    if (-not $TOKENS["AdminCoppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $rPepsi = Invoke-Api -Url "$BASE/api/v1/admin/tech-support?pageNumber=1`&pageSize=100" -Token $TOKENS["AdminCoppelia"]
    $rCoca  = Invoke-Api -Url "$BASE/api/v1/admin/tech-support?pageNumber=1`&pageSize=100" -Token $TOKENS["AdminSavory"]
...
Test-Case "MS06" "GET /admin/mermas Admin Pepsi no ve mermas de Coca" {
    if (-not $TOKENS["AdminCoppelia"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/mermas?pageNumber=1`&pageSize=10" -Token $TOKENS["AdminCoppelia"]
...
Test-Case "PR05" "POST /auth/reset-password con token invalido retorna 400 o 404" {
    $r = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/reset-password" -Body @{
...
Test-Case "SEC04" "Token de un tenant no funciona en otro tenant" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/admin/dashboard" -Token $TOKENS["AdminSavory"]
    $rPepsiEndpoint = Invoke-Api -Url "$BASE/api/v1/users" -Token $TOKENS["AdminCoppelia"]
...
Test-Case "SEC05" "Payload JSON malformado retorna 400 (no 500)" {
    ...
    try {
        $resp = Invoke-WebRequest -Method POST -Uri "$BASE/api/v1/auth/login" `
...
Test-Case "SEC08" "GET /api/v1/../../../etc/passwd retorna 400 o 404 (no 500)" {
    $r = Invoke-Api -Url "$BASE/api/v1/../../../etc/passwd"
...
Test-Case "PF02" "GET /users paginado responde en menos de 300ms" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=1`&pageSize=20" -Token $TOKENS["AdminSavory"]
    $sw.Stop()
...
Test-Case "PF04" "pageSize=100 responde en menos de 1000ms" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=1`&pageSize=100" -Token $TOKENS["AdminSavory"]
    $sw.Stop()
...
Test-Case "PF05" "10 requests concurrentes a /ping no producen error" {
    $jobs = 1..10 | ForEach-Object {
        Start-Job -ScriptBlock {
            param($base)
            try {
                $resp = Invoke-WebRequest -Uri "$base/api/v1/ping" -UseBasicParsing -TimeoutSec 10 -EA Stop
...
Test-Case "PF06" "pageNumber negativo retorna 400 o trata como pagina 1 (no 500)" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=-1`&pageSize=10" -Token $TOKENS["AdminSavory"]
...
Test-Case "PF07" "pageSize=0 retorna 400 o usa minimo (no 500 ni division por cero)" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/users?pageNumber=1`&pageSize=0" -Token $TOKENS["AdminSavory"]
...
Test-Case "CS01" "Dos logins del mismo usuario generan tokens distintos" {
    $r1 = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/login" -Body @{
        email=$CREDS["AdminSavory"].email; password=$SQL_PASS; tenantSlug=$DB.T1.Slug
    }
    Start-Sleep -Milliseconds 100
    $r2 = Invoke-Api -Method POST -Url "$BASE/api/v1/auth/login" -Body @{
        email=$CREDS["AdminSavory"].email; password=$SQL_PASS; tenantSlug=$DB.T1.Slug
    }
...
Test-Case "CS02" "Token revocado via logout no puede usarse en endpoints" {
    ...
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/home" -Token $token
...
Test-Case "CS03" "Admin de tenant A con token valido no accede a datos de tenant B" {
    ...
    $rCoca  = Invoke-Api -Url "$BASE/api/v1/users" -Token $TOKENS["AdminSavory"]
    $rPepsi = Invoke-Api -Url "$BASE/api/v1/users" -Token $TOKENS["AdminCoppelia"]
...
Test-Case "CS04" "GET /auth/me o /profile retorna datos del usuario autenticado" {
    if (-not $TOKENS["AdminSavory"]) { return @{ ok=$false; detail="Sin token" } }
    $candidates = @("$BASE/api/v1/auth/me", "$BASE/api/v1/profile", "$BASE/api/v1/users/me")
    $found = $false
...
Test-Case "CS05" "Login con credenciales correctas actualiza LastLoginAt en BD" {
    ...
    Invoke-Api -Method POST -Url "$BASE/api/v1/auth/login" -Body @{
        email=$CREDS["AdminSavory"].email; password=$SQL_PASS; tenantSlug=$DB.T1.Slug
    } | Out-Null
...
Test-Case "HUB01" "Cliente GET /products incluye campo stockExterno" {
    if (-not $TOKENS["Cliente1Savory"]) { return @{ ok=$false; detail="Sin token" } }
    $r = Invoke-Api -Url "$BASE/api/v1/cliente/products" -Token $TOKENS["Cliente1Savory"]
...
Test-Case "TD01" "Eliminar usuarios de prueba generados" {
    ...
    $done = 0
    foreach ($u in $toDelete) {
        $del = Invoke-Api -Method DELETE -Url "$BASE/api/v1/users/$($u.id)" -Token $TOKENS["AdminSavory"]
